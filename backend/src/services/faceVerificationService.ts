...
export class FaceVerificationService {
   ...
   * Useful for detecting photo substitution attacks
   */
  async verifyFaceConsistency(
    faceImages: Buffer[]
  ): Promise<{
    ...
    if (faceImages.length < 2) {
      return { isConsistent: true, averageConfidence: 1.0 };
    }

    try {
      await this.loadModels();

      console.log(`[FaceVerificationService] Checking consistency across ${faceImages.length} images`);

      // Get face descriptors for all images
      const descriptors: Float32Array[] = [];
      
      for (let i = 0; i < faceImages.length; i++) {
        const img = await this.bufferToImage(faceImages[i]);
        ...
        if (!detection) {
          console.warn(`[FaceVerificationService] No face detected in image ${i + 1}`);
          return { isConsistent: false, averageConfidence: 0 };
        }
      ...
      const averageConfidence = comparisons > 0 ? totalScore / comparisons : 1.0;
      const isConsistent = pairwiseScores.every((_, idx) => {
        const distance = 1 - pairwiseScores[idx];
        return distance < this.matchThreshold;
      });

      console.log(`[FaceVerificationService] Consistency check: ${isConsistent ? 'PASSED' : 'FAILED'}, average confidence: ${averageConfidence.toFixed(4)}`);
      ...
      console.error('[FaceVerificationService] Error checking consistency:', error);
      return { isConsistent: false, averageConfidence: 0 };
    }
  }

  /**
   * Get face descriptor (embedding) for a face image
   * Useful for storing and comparing faces later
   */
  async getFaceDescriptor(imageBuffer: Buffer): Promise<Float32Array | null> {
    try {
      await this.loadModels();
      ...
      console.error('[FaceVerificationService] Error getting face descriptor:', error);
      return null;
    }
  }
}
